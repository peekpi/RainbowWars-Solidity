pragma solidity ^0.5.0;
pragma experimental ABIEncoderV2; // solium-disable-line no-experimental

import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/ownership/Ownable.sol";
import "../nearbridge/INearBridge.sol";
import "../nearbridge/NearDecoder.sol";
import "./ProofDecoder.sol";
//import "./INearProver.sol";


contract NearProver {
    using SafeMath for uint256;
    using Borsh for Borsh.Data;
    using NearDecoder for Borsh.Data;
    using ProofDecoder for Borsh.Data;
    using ProofDecoder for bytes;

    INearBridge public bridge;

    function _revert(string memory pre, bytes memory num) private {
        bytes memory hexStr = bytes("0123456789abcdef");
        bytes memory preb = bytes(pre);
        bytes memory message = new bytes(preb.length + num.length*2);
        bool start = false;
        for(uint i = 0; i < preb.length; i++) message[i] = preb[i];
        for(uint i = 0; i < num.length; i++) {
            uint pos = preb.length + i*2;
            uint chr = uint8(num[i]);
            message[pos] = hexStr[chr>>4];
            message[pos+1] = hexStr[chr&0xf];
        }
        revert(string(message));
    }

    function proveOutcome(bytes memory proofData, uint64 blockHeight) internal returns(bytes32, bytes32, bytes memory) {
        Borsh.Data memory borshData = Borsh.from(proofData);
        ProofDecoder.FullOutcomeProof memory fullOutcomeProof = borshData.decodeFullOutcomeProof();
    
        require(borshData.finished(), "NearProver: argument should be exact borsh serialization");

        bytes32 hash = _computeRoot(
            fullOutcomeProof.outcome_proof.outcome_with_id.hash,
            fullOutcomeProof.outcome_proof.proof
        );

        hash = sha256(abi.encodePacked(hash));

        hash = _computeRoot(
            hash,
            fullOutcomeProof.outcome_root_proof
        );

        require(
            hash == fullOutcomeProof.block_header_lite.inner_lite.outcome_root,
            "NearProver: outcome merkle proof is not valid"
        );
        //_revert("return:",fullOutcomeProof.outcome_proof.outcome_with_id.outcome.status.successValue);
        //_revert("receiptRoot:",abi.encode(hash));
        /*
        bytes32 expectedBlockMerkleRoot = bridge.blockMerkleRoots(blockHeight);

        if (expectedBlockMerkleRoot != fullOutcomeProof.block_header_lite.inner_lite.block_merkle_root) {
            require(
                _computeRoot(fullOutcomeProof.block_header_lite.hash, fullOutcomeProof.block_proof) == expectedBlockMerkleRoot, "NearProver: block proof is not valid"
            );
        }
        */

        bytes32 receipt = fullOutcomeProof.outcome_proof.outcome_with_id.outcome.receipt_ids[0];
        bytes32 bridge = keccak256(fullOutcomeProof.outcome_proof.outcome_with_id.outcome.executor_id);
        ProofDecoder.ExecutionStatus memory status = fullOutcomeProof.outcome_proof.outcome_with_id.outcome.status;
        require(!status.failed, "Cannot use failed execution outcome for unlocking the tokens.");
        require(!status.unknown, "Cannot use unknown execution outcome for unlocking the tokens.");
        return (bridge, receipt, status.successValue);
    }

    function _computeRoot(bytes32 node, ProofDecoder.MerklePath memory proof) internal pure returns(bytes32 hash) {
        hash = node;
        for (uint i = 0; i < proof.items.length; i++) {
            ProofDecoder.MerklePathItem memory item = proof.items[i];
            if (item.direction == 0) {
                hash = sha256(abi.encodePacked(item.hash, hash));
            }
            else {
                hash = sha256(abi.encodePacked(hash, item.hash));
            }
        }
    }
}
